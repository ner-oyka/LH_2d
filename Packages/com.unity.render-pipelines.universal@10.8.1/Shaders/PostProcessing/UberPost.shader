Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_local_fragment _ _EDGESBLUR
        ...
        #define EdgesBlurPower          _EdgesBlur_Params1.x
        #define EdgesBlurDarkness       _EdgesBlur_Params1.y
        #define EdgesBlurDistance       _EdgesBlur_Params1.z
        #define EdgesBlurHardness       _EdgesBlur_Params1.w
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            // Edges Blur
            if (EdgesBlurPower > 0.0f)
            {
                half3 col;
                float invAspect = _ScreenParams.y / _ScreenParams.x;
                for(float index=0;index<10;index++)
                {
                    float offset = (index/9 - 0.5) * EdgesBlurPower * 0.01f;
                    float2 uv2 = uv + float2(offset * invAspect, 0);
                    float stDevSquared = 0.4f*0.4f;
                    float gauss = (1 / sqrt(2*PI*stDevSquared)) * pow(2.71828182846f, -((offset*offset)/(2*stDevSquared)));

                    col += SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv2) * gauss;
                }
                
                float2 center = UnityStereoTransformScreenSpaceTex(VignetteCenter);
                float2 dist = abs(uv - center) * EdgesBlurDistance;

                #if defined(UNITY_SINGLE_PASS_STEREO)
                dist.x /= unity_StereoScaleOffset[unity_StereoEyeIndex].x;
                #endif

                dist.x *= EdgesBlurHardness;
                float vfactor = pow(saturate(dot(dist, dist)), EdgesBlurDarkness);
                color = lerp(color, col / 10, vfactor);
            } 
